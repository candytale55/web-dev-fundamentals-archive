/* This is loaded in when the user clicks the mouse in*/



/*              VARIABLES             */

//  timeLeft is the number of seconds we will allot for gameplay and will be used to track how many seconds are left as the game progresses.
let timeLeft = 30;

// score keeps track of the points.
let score = 0;

// We have a pause feature. The game is not paused when it starts, so initial value is false
let isPaused = false;

// currentBurrowKey stores the key of the burrow that the mole is currently residing in.
...
class GameScene extends Phaser.Scene {
  ...
  /*             CREATE ()               */
  /* Sets up scene visuals, animations, and game logic when events occur. */

  create() {
		// updateTimer() function sets timer to count down, timeLeft decrements after each second passes so that the game can end.
    const updateTimer = () =>{
      timeLeft -= 1;
    }
    
    // onSecondElapsed() function is set up to execute after every second passes.
		const onSecondElapsed = () => {
			if (isPaused === false) {
        updateTimer(); // diminishes timeLeft each second.
  ...
  /*               UPDATE ()         */
	...
	update() {
		...
		// applyHitReward() is when user successfully hit the mole, so reward the user with 5pts. Rewards display next to the score box. 
    ...
    // onBurrowHit() will run when the user presses the J, K, or L  and it will distribute rewards and penalties accordingly.
    const onBurrowHit = (key) => {
    // console.log(key); // Testing hit the J, K, or L key, and see the corresponding key log out in the console. They are called bellow on the IF statements. It displays a Reference:Error.
    }
  ...
  // gameState.jKey, kKey and lKey, will identify when the user hits one of the keys
       if (Phaser.Input.Keyboard.JustDown(gameState.jKey)) {
        onBurrowHit(j);
        applyHitReward();
        this.relocateMole(); // will move the mole to another location 
