...
let monsterHealth;
let inventory = ["stick"]; // Starting weapon
...
// Game locations with corresponding button text, functions, and description
const locations = [
    {
        name: "town square",
...
// Set up initial button functions
button1.onclick = goStore;
button2.onclick = goCave;
button3.onclick = fightDragon;

// Updates UI with location info and button setup
...
function sellWeapon() {
    if (inventory.length > 1) {
        gold += 15;
        goldText.innerText = gold;
        let currentWeapon = inventory.shift(); // Remove and return the first weapon
...
// Monster encounter setup functions
...
function attack() {
    ...
    // 10% chance to break a weapon (unless only one remains)
...
// Calculates how much damage a monster deals
...
// Returns true if attack hits (80% chance, or always if health is low)
...
// Rewards player after defeating a monster
